    ...
    while True:
        ...
        if(len(aligns) > 0):
            ...
            for (i,rect) in enumerate(rects):
                ...
                cv2.putText(frame,recog_data[i][0]+" - "+str(recog_data[i][1])+"%",(rect[0],rect[1]),cv2.FONT_HERSHEY_SIMPLEX,1,(255,0,255),1,cv2.LINE_AA)
...
def create_manual_data():
    ...
    data_set = json.loads(f.read());
    # trích xuất điểm dặc trưng của face muốn train
